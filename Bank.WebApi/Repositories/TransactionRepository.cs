...
namespace Bank.WebApi.Repositories
{
    public class TransactionRepository
    {
        ...
        {
            string sql = @$"INSERT INTO transactions (account_id, amount, cause) 
                        VALUES (@AccountId, @amount, @reason)";
            await _connection.ExecuteAsync(sql, new { AccountId, amount, reason });
        }
        public async Task<IEnumerable<TransactionEntity>> Get()
        {
            string sql = @$"SELECT id, account_id AS AccountId, amount, cause AS description, created_at 
                        FROM transactions";
